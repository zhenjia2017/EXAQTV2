...
class CustomTrainer(Trainer):
    def __init__(
        ...
        path_to_best_model= f"_data/timequestionsv2/fact_scoring",
    ):
        super().__init__(
            model,
            args,
    ...
    def evaluate(
        ...
        eval_metric = eval_output.metrics["eval_loss"]

        # store model if performance improved
        if self.state.best_model_checkpoint is None or eval_metric < self.state.best_metric:
