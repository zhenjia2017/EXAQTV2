...
class AnswerGraph:
    ...
    def path_inference_on_data_split(self, input_path, output_path):
        ...
        with open(input_path, "r") as fi, open(output_path, "w") as fp:
            ...
            for instance in datas[::-1]:
                if instance["Id"] == 4468 or instance["Id"] == 3467: break
    ...
    def temporal_ers_inference(self):
        ...
        # process data
        input_path = os.path.join(input_dir, self.nerd, f"test-ers-{fs_max_evidences}-gst-{topg}.jsonl")
        output_path = os.path.join(output_dir, self.nerd, f"test-ers-{fs_max_evidences}-gst-{topg}-{tempfs_max_evidences}.jsonl")

        self.tempers_inference_on_data_split(input_path, output_path)

        input_path = os.path.join(input_dir, self.nerd, f"dev-ers-{fs_max_evidences}-gst-{topg}.jsonl")
        output_path = os.path.join(output_dir, self.nerd, f"dev-ers-{fs_max_evidences}-gst-{topg}-{tempfs_max_evidences}.jsonl")

        self.tempers_inference_on_data_split(input_path, output_path)
        #
        # input_path = os.path.join(input_dir, self.nerd, f"train-ers-{fs_max_evidences}-gst-{topg}.jsonl")
        # output_path = os.path.join(output_dir, self.nerd, f"train-ers-{fs_max_evidences}-gst-{topg}-{tempfs_max_evidences}.jsonl")
        #
        # self.tempers_inference_on_data_split(input_path, output_path)
    ...
    def evaluate_retrieval_results(self, results_path):
        ...
        with open(results_path, 'r') as fp:
            for line in tqdm(fp):
                instance = json.loads(line)
                print (instance["Temporal question type"])
                #category_slot = [cat.lower() for cat in instance["Temporal question type"]]
                category_slot = [instance["Temporal question type"]]
                print (category_slot)
                #if "ordinal" in category_slot: continue
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        raise Exception("python exaqt/answer_graph/answer_graph_construction.py <PATH_TO_CONFIG>")

    # load config
    config_path = sys.argv[1]

    erp = AnswerGraph(config_path)

    input_dir = erp.config["path_to_intermediate_results"]
    fs_max_evidences = erp.config["fs_max_evidences"]
    nerd = erp.config["nerd"]
    # process data
    fs_max_evidences = erp.config["fs_max_evidences"]
    topg = erp.config["top-gst-number"]
    tempfs_max_evidences = erp.config["temporal_fs_max_evidences"]
    input_path = os.path.join(input_dir, nerd,
                               f"test-ers-{fs_max_evidences}-gst-{topg}-{tempfs_max_evidences}.jsonl")
    #input_path = os.path.join(input_dir, nerd, f"test-ers-{fs_max_evidences}.jsonl")

    erp.evaluate_tempers_inference_results(input_path)
