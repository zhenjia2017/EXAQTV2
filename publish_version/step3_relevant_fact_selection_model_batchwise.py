...
def predict_scores(self, ques, tuple):
    """
    Retrieve relevance scores for the given evidences
    for the current AR. The process is done in batches
    for efficiency.
    """
    test_batch_size = self.config["evs_test_batch_size"]
    ar = abstract_representation[0]
    print(f"ar: {ar}")
    ...
    for i in range(number_of_batches):
        ...
        if not batch_evidences:
            break
        batch_evidence_texts = [evidence_to_text(evidence) for evidence in batch_evidences]
        ars = [ar] * len(batch_evidences)

        # concat new relevance scores for batch
        if relevances is None:
            relevances = self.ers.inference(ars, batch_evidence_texts)
        else:
            relevances = torch.cat((relevances, self.ers.inference(ars, batch_evidence_texts)))
    print(f"Finished! Total batches: {number_of_batches}, time consumed: {time.time() - start_time}")
    return relevances
...
class FactScoring(torch.nn.Module):
    def __init__(self, cfg):
        super(FactScoring, self).__init__()
        self.tokenizer = TOKENIZER
        self.max_len = MAX_LEN
        self.config = cfg
    ...
    def load(self):
        """Load model."""
        path = os.path.join(self.model_path, "phase1_model_new.bin")
    ...
    def inference(self, ques, contexts):
    ...
    def train(self, TRAINING_FILE, MODEL_PATH):
