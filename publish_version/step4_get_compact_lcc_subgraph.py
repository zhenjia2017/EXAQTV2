...
def get_xg_GRAPH(graph_file, comgstspo, pro_info):
    spo_fact = {}
    ent_sta = {}
    unique_SPO_dict = {}
    sta_score = {}

    if len(comgstspo) == 0: return
    for line in comgstspo:
        ...
        if statement_id not in sta_score:
            sta_score[statement_id] = 0.0
        n1_id = triple[1]
        if "corner#" in n1_id: n1_id = n1_id.split("#")[1]
        n1_name = triple[2]
        n2 = triple[4]
        n3_id = triple[5]
        if "corner#" in n3_id: n3_id = n3_id.split("#")[1]
        n3_name = triple[6]
    ...
    XG = build_graph_from_triple_edges(unique_SPO_dict, pro_info, sta_score)
    del spo_fact
    del unique_SPO_dict
    XG = directed_to_undirected(XG)
    #nx.write_gpickle(XG, graph_file)
    return XG
...
class QuestionUnionGST():
    ...
    def get_QKG_GST_from_spo(self, id, question, gt, connect, seeds_paths, add_spo_line, add_spo_score):
        ...
        graph_file = path + '/LCCQKG_' + connect + str(self.topf) + '.gpickle'
        corner_file = path + '/LCCcornerstone_' + connect + str(self.topf)
        unionGST_file = path + '/LCCGST_' + connect + str(self.topf) + '_' + str(self.topg) + '.gpickle'
        ...
        try:
            ...
            GST = call_main_rawGST(G, cornerstone, self.topg)
        ...
        nx.write_gpickle(GST, unionGST_file)
        gst_number_of_components_connect = nx.number_connected_components(GST)
        gst_can_entities = [node.split("::")[2].replace('T00:00:00Z', '').lower() for node in GST.nodes() if
                                node.split("::")[1] == "Entity"]
        union_predicates = [node.split("::")[0] + "::" + node.split("::")[2].lower() for node in GST.nodes
                                 if node.split("::")[1] == "Predicate"]
        union_cornerpredicates = [node.split("::")[2].lower() for node in GST.nodes if
        ...
        GST_node_len = len(GST.nodes())
        GST_edge_len = len(GST.edges())
...
if __name__ == "__main__":
    ...
    analysis_file = cfg["data_path"] + 'analysis/' + dataset + '_' + str(topf) + '_'  + str(topg) + connect + 'add_LCCcompact_subgraph_analysis.txt'
    ...
    qkgspo_file = qkg_path + "/" + dataset + '_' + str(topf) + "LCC.json"
    gstspo_file = gst_path + "/" + dataset + '_' + str(topf) + '_'  + str(topg) + "LCC.json"
