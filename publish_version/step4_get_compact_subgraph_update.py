...
def call_main_GRAPH(spo_file, spo_rank_file, rank, q_ent, pro_info, seeds_paths, add_spo_line, add_spo_score):
...
class QuestionUnionGST():
    ...
    def get_completedgst(self, QKG, unionGST, cornerstone):
    ...
    def get_QKG_GST_from_spo(self, id, question, gt, connect, seeds_paths, add_spo_line, add_spo_score):
        ...
        completedGST_file = path + '/completedGST_' + connect + str(self.topf) + '_' + str(self.topg) + '.gpickle'
        ...
        QKG, cornerstone, qkgspo, number_of_components, number_of_components_connect = call_main_GRAPH(spo_file,
                                                                                                       spo_rank_file,
                                                                                                       self.topf, q_ent,
                                                                                                       pro_info,
                                                                                                       seeds_paths,
        ...
        if len(GST_list) > 0:
            ...
            unionGST_clean = nx.Graph()
            unionGST_prune = nx.Graph()
            #add nodes from union of GSTs but remove the hanging predicates
            for n in unionGST.nodes():
                nn = n.split('::')
                if nn[1] == 'Predicate' and n not in cornerstone and unionGST.degree(n) == 1:
                    continue
                unionGST_clean.add_node(n)
            #add edges from union of GSTs
            for (n1, n2) in unionGST.edges():
                if n1 in unionGST_clean and n2 in unionGST_clean:
                    unionGST_clean.add_edge(n1, n2)
            #remove hanging predicates from cleaned union of GSTs
            for n in unionGST_clean.nodes():
                nn = n.split('::')
                if nn[1] == 'Predicate' and n not in cornerstone and unionGST.degree(n) == 1:
                    unionGST_clean.remove_node(n)
            #remove independent entities
            unionGST_prune = unionGST_clean.copy()
            for n in unionGST_clean.nodes():
                nn = n.split('::')
                if nn[1] == 'Entity' and n not in cornerstone and unionGST_clean.degree(n) == 0:
                    unionGST_prune.remove_node(n)

            for n in unionGST_clean.nodes():
                if nn[1] == 'Entity' and n in cornerstone and unionGST_clean.degree(n) == 0:
                    for neighbor in unionGST.neighbors(n):
                        unionGST_prune.add_node(neighbor)
                        unionGST_prune.add_edge(n, neighbor)

            #print("\n\nSize of the unionGST", len(unionGST.nodes()), len(unionGST.edges()))
            completedGST = self.get_completedgst(QKG, unionGST_prune, cornerstone)
            nx.write_gpickle(unionGST_prune, unionGST_file)
            nx.write_gpickle(completedGST, completedGST_file)
            ...
            complete_gstspo_entities = [node.split("::")[2].replace('T00:00:00Z', '').lower() for node in completedGST.nodes() if
                                node.split("::")[1] == "Entity"]
...
if __name__ == "__main__":
    ...
    analysis_file = cfg["data_path"] + 'analysis/' + dataset + '_' + str(topf) + '_'  + str(topg) + connect + 'add_compact_subgraph_analysis_nochainjoin_nohangpredicate.txt'
    ...
    qkg_path = cfg["data_path"] + connect + "_qkg_nochainjoin_nohangpredicate"
    gst_path = cfg["data_path"] + connect + "_xg_nochainjoin_nohangpredicate"
    os.makedirs(qkg_path, exist_ok=True)
    os.makedirs(gst_path, exist_ok=True)
    qkgspo_file = qkg_path + "/" + dataset + '_' + str(topf) + "_nochainjoin_nohangpredicate.json"
    gstspo_file = gst_path + "/" + dataset + '_' + str(topf) + '_'  + str(topg) + "_nochainjoin_nohangpredicate.json"
