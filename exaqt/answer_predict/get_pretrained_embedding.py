...
from exaqt.answer_predict.time_encoder import TimeEncoder
from exaqt.answer_predict.util import load_dict
...
def tempfact_emb(sorted_tkgfacts, entity2id, relation2id, entity_embeddings, relation_embeddings, date_embeddings):
    ...
    for i in range(len(reverse_tempfact2id)):
        ...
        if "T00:00:00Z" in tuple[2]:
            replace_tuple = tuple[2].replace("T00:00:00Z","")
        else:
            replace_tuple = tuple[2]
        obj_emb = entity_embeddings[str(entity2id[replace_tuple])]
...
def get_pretrained_embedding_from_wiki2vec(MODEL_FILE, answer_predict_path):
    ...
    fp_entity = os.path.join(answer_predict_path, 'entities.txt')
    fp_entity_name_map = os.path.join(answer_predict_path, 'entity_name_map.pkl')
    fp_date = os.path.join(answer_predict_path, 'dates.txt')
    fp_relation = os.path.join(answer_predict_path, 'relations.txt')
    fp_word = os.path.join(answer_predict_path, 'words.txt')
    fp_tempfact_pkl = os.path.join(answer_predict_path, 'tempfacts.pkl')

    fp_tempfact2id_pkl =  os.path.join(answer_predict_path, 'tempfacts2id.pkl')
    relation_emb_file = os.path.join(answer_predict_path, 'relation_emb_100d.npy')
    vocab_emb_file = os.path.join(answer_predict_path, 'word_emb_100d.npy')
    entity_emb_file = os.path.join(answer_predict_path, 'entity_emb_100d.npy')
    entity_date_emb_file = os.path.join(answer_predict_path, 'date_te_emb_100d.npy')
    tempfact_te_emb_file = os.path.join(answer_predict_path, 'tempfact_te_emb_500d.npy')
    tempfact_emb_file = os.path.join(answer_predict_path, 'tempfact_emb_400d.npy')
