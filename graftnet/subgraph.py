    ...
    for statement_id in state:
        ...
        if len(state[statement_id]['ps']) > 0 and len(state[statement_id]['pq']) > 0:
            ...
            for line in ps_lines:
                ...
                if rel_name in t_rels and obj not in t_ens: t_ens.add(obj)
                # if rel_name in t_rels:
                #     print((ps_obj, rel_name, obj))
                    #t_triple.append((ps_obj, rel_name, obj))
            ...
            for line in pq_lines:
                ...
                if rel_name in t_rels and obj not in t_ens: t_ens.add(obj)
                # if rel_name in t_rels:
                #     print((pq_sub, rel_name, obj))
                    #t_triple.append((pq_sub, rel_name, obj))
...
#         return False
#     all_entities = set([inv_map[ee] for ee, _ in entities])
#     answer_candidate_ids = {answer_candidate_id: answer_candidate_id.lower().strip().replace('"', "").replace("+", "")
#                             for answer_candidate_id in all_entities}
#
#     for answer in GT:
#         gold_answer_id = answer["id"].lower().strip().replace('"', "").replace("+", "")
...
class SubGraphGenerator:
    ...
    # def generate_subgraph(self, input_path, output_path, dataset):
    #     seed_map = {}
    #     answer_recall = []
    #     total = 0
    #     bad_questions = []
    ...
    #     total_entities = 0
    #     SPONOTFOUND = 0
    #     relation_embeddings = pkl.load(open(self.relation_emb_pkl_file, 'rb'))
    #     question_embeddings = pkl.load(open(self.question_emb_pkl_file, 'rb'))
    #     self.logger.info(f"Input data loaded from: {input_path}.")
    #     with open(input_path, 'r') as fp, open(output_path, "wb") as fo:
    #         for line in tqdm(fp):
    ...
    #             QuestionText = instance["Question"]
    #             GT = get_answer(instance["answers"])
    #             question_embedding = question_embeddings[QuestionId]
    #             seed_map[QuestionId] = _read_seeds(instance, self.nerd)
    #             tuples, t_rels, t_ents, line_map, entity_name = get_triple_from_SPO(evidences, self.property)
    #
    #             if not (tuples):
    ...
    #                 entity_map, relation_map, adj_mat = {}, {}, None
    #             else:
    #                 entity_map, relation_map, adj_mat = _read_facts(tuples, relation_embeddings, question_embedding)
    #
    #             inv_map = {i: k for k, i in entity_map.items()}
    #             seed_entities = []
    #             ans_entities = []
    ...
    #
    #
    #         print("%d questions with empty subgraphs." % num_empty_tuples)
    #         print("Number of OK questions = %s" % str(len(ok_questions)))
    #         print("Answer recall = %.3f" % (sum(answer_recall) / len(answer_recall)))
    #
